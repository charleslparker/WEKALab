...
public class F1Split extends ClassifierSplitModel {
    /*
    ...
    /** for serialization */
    private static final long serialVersionUID = -1278776912563022474L;
    ...
    private static F1SplitCrit m_F1Crit = new F1SplitCrit();

    /**
     * Initializes the split model.
     */
    public F1Split(int attIndex,int minNoObj,double sumOfWeights){
        ...
        for (i = 0; i < numAttValues; i++){
            ...
            if (Utils.grOrEq(newDistribution.perBag(i),m_minNoObj)){
                ...
                if (secondDistribution.check(m_minNoObj)){
                    m_numSubsets = 2;
                    currF1 = m_F1Crit.splitCritValue(secondDistribution);
                    if ((i == 0) || Utils.gr(currF1, m_f1)){
                        m_f1 = currF1;
    ...
    throws Exception {
        ...
        while (next < firstMiss){
            ...
            if (trainInstances.instance(next-1).value(m_attIndex)+1e-5 < 
                ...
                if (Utils.grOrEq(m_distribution.perBag(0),minSplit) && 
                    ...
                    currentInfoGain = m_F1Crit.splitCritValue(m_distribution);
                    if (Utils.gr(m_f1, currentInfoGain)){
                        m_f1 = currentInfoGain;
                        splitIndex = next-1;
                    }
                    index++;
                }
        ...
        m_f1 = m_F1Crit.splitCritValue(m_distribution);
    }
